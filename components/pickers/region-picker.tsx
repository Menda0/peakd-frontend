...
export function RegionPicker({
  ...
  const submitNewRegion = async () => {
    if (!countryCode || !newName.trim()) return;
    setCreateError(null);
    ...
    try {
      ...
      const res = await fetch(`${base}/studio/regions`, {
        ...
        body: JSON.stringify({ countryCode, name: newName.trim() }),
      });
      if (!res.ok) {
        throw new Error(await res.text().catch(() => res.statusText));
      }
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <ComboboxInput
          id={id}
          placeholder={countryCode ? "Search region…" : "Select a country first"}
          disabled={gated || loading}
          className={cn(
